...
const kTpuNonCorePlaneNamePrefix = "#Chip";
...
pub fn findHostEventType(event_name: []const u8) ?i64 {
    if (HostEventTypeMap.get(event_name)) |event_type| return @intFromEnum(event_type);
    return null;
}

pub fn findStatType(stat_name: []const u8) ?i64 {
    if (StatTypeMap.get(stat_name)) |stat_type| return @intFromEnum(stat_type);
    return null;
}

pub fn findTfOpEventType(event_name: []const u8) ?i64 {
    return switch (tf_op_utils.parseTfOpFullName(event_name).category) {
        .kTensorFlow => @intFromEnum(HostEventType.kTfOpRun),
        .kTfData => @intFromEnum(HostEventType.kIterator),
        else => null,
    };
}
